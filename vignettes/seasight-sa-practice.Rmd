...
In Part I, calendar adjustment was framed as a two-step decision:
economic plausibility and statistical evidence. The same logic underpins
the TD and Easter handling in **seasight**.

When you call `auto_seasonal_analysis()`, you specify the *type* of
trading-day adjustment you are willing to consider (if any):
...
Internally, seasight compares candidates **with** and **without**
calendar regressors and records the impact on diagnostics and fit. In
the candidate table you can inspect, for example, whether the preferred
model uses TD and Easter:
...
The text calls in `res_td$seasonality` summarise the recommended
treatment:
...
This allows you to line up the **economic expectation** (e.g. “TD should
matter for production, but probably not for this ratio series”) with the
statistical evidence, and to document the final choice.
