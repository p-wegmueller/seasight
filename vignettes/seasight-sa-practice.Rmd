...
#> - call_overall     : "ADJUST", "BORDERLINE" or "DO_NOT_ADJUST"
#> - share_ids        : share of candidates with identifiable seasonality
#> - share_qsori      : share with QS evidence on the original series
#> - share_m7_strong  : share with strong M7 evidence
```
...
head(res$table[
  ,
  c("model_label", "arima", "M7", "IDS", "QS_p", "LB_p", "rev_mae")
])
```
...
* core diagnostics (`M7`, `IDS`, `QS_p`, `LB_p`),
...
The first row records the selected treatment. The overall seasonality component
records whether the series should be adjusted, reviewed as borderline, or left
unadjusted:

```{r sa-td-calls, eval = FALSE}
res_td$seasonality$overall$call_overall
```
...
comparison of baseline and best model, including diagnostics and aligned
seasonally adjusted and seasonal-component series where available:

```{r sa-compare, eval = FALSE}
cmp <- sa_compare(res_baseline, m_current)
cmp
cmp$summary
```
...
> * `sa_compare()` summarises the impact of switching in terms of diagnostics and aligned series.
...
sa_report_html(
  res,
  file = tempfile("AirPassengers_seasight_report-", fileext = ".html")
)
#> Opens an HTML report with:
#> - key diagnostics
#> - top candidates table
#> - plots for original and seasonally adjusted series
...
sa_copyable_call(res$best, x_expr = "y")
