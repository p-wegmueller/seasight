...
Internally, it uses the *best* row of `res$table`. The rule checks:
...
* **Similarity of seasonal pattern:**
  The seasonal factors should remain sufficiently correlated with the
  incumbent when an incumbent comparison is available.

* **Distance to incumbent:**
  The candidate's seasonally adjusted series should not be an outlying distance
  from the incumbent relative to the candidate set.

The broader ranking step in `auto_seasonal_analysis()` also uses AICc, revision
metrics for the top candidates, distance-to-incumbent diagnostics and an engine
preference penalty. Those quantities determine which row is considered "best";
`sa_should_switch()` then applies the narrower switching gate to that row.
