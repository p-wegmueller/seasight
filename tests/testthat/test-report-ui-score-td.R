...
test_that("top candidates table names holiday regressors explicitly", {
  tbl <- tibble::tibble(
    model_label = c("best", "runner"),
    arima = c("(0 1 1)(0 1 1)", "(1 1 1)(0 1 1)"),
    with_td = c(TRUE, FALSE),
    td_name = c("diwali", NA_character_),
    td_label = c("diwali", NA_character_),
    score_100 = c(97, 80),
    AICc = c(100, 102),
    LB_p = c(0.30, 0.20),
    QSori_p = c(0.01, 0.02),
  ...
  expect_match(html, "TD regressor", fixed = TRUE)
  expect_match(html, "diwali", fixed = TRUE)
  expect_match(html, "none", fixed = TRUE)
})
